...
static OQS_STATUS kem_test_key_derive(const char *method_name) {
	...
	if ((public_key_1 == NULL) || (secret_key_1 == NULL) || (public_key_2 == NULL) || (secret_key_2 == NULL) || (seed == NULL)) {
	...
	OQS_randombytes(seed, 64);

	rc = OQS_KEM_derive_keypair(kem, seed, public_key_1, secret_key_1);
	if (rc != OQS_SUCCESS) {
		fprintf(stderr, "ERROR: OQS_KEM_derive_keypair failed\n");
		goto err;
	}

	rc = OQS_KEM_derive_keypair(kem, seed, public_key_2, secret_key_2);
	if (rc != OQS_SUCCESS) {
		fprintf(stderr, "ERROR: OQS_KEM_derive_keypair failed\n");
		goto err;
	}
